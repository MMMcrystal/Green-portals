...
function init() {//初始化
	...
	var darkGreenMaterial = new THREE.MeshPhongMaterial({ color: 0xffffff, shading: THREE.FlatShading, side: THREE.DoubleSide, vertexColors: THREE.FaceColors });
	...
	plane = new THREE.Mesh(geometry, darkGreenMaterial);
	scene.add(plane);
...
function createStars(amount, yDistance) {//星空
...
function render() {//随机颜色
	...
	if (intersects.length > 0) {
		(function () {
			...
			intersects[0].face.color.setStyle("#4ECDC4");
			plane.geometry.colorsNeedUpdate = true;
		})();
	}
...
$('.shift-camera-button').click(function () {//进入按钮
